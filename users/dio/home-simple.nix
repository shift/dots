# Simple Working Configuration for dio User
{ pkgs, inputs', ... }:
{
  imports = [
    # Use individual DOTS modules that work - no default module
    ...
    inputs'.dots.homeManagerModules.hardware-aware-dev-env
    inputs'.dots.homeManagerModules.dynamic-notifications
  ];

  # Define features option manually since we're not using default module
  features = {
    dynamic-aliases = {
      enable = true;
      aliases = [
    ...
    dynamic-autostart = {
      enable = true;
      applications = [
        { name = "nm-applet"; command = "nm-applet"; }
        { name = "udiskie"; command = "udiskie --tray"; }
      ];
    };
